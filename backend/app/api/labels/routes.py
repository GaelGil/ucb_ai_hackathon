...
from app.api.container import AppServices, get_labels_service, get_services
...
@bp.get("/datasets/<dataset_id>/annotation-rows")
def list_annotation_rows(dataset_id: str):
    row_type = _enum_arg(SuggestionType, "type")
    if row_type != SuggestionType.POS:
        json_abort(400, "Only POS annotation rows are supported.")
    limit = _int_arg("limit", 10, 1, 100)
    offset = _int_arg("offset", 0, 0, 10**9)
    needs_review = _bool_arg("needs_review")
    rows, total = get_labels_service().list_annotation_rows(
        dataset_id,
        row_type,
        limit,
        offset,
    ...
    return json_response(AnnotationRowsResponse(rows=rows, total=total, limit=limit, offset=offset))
