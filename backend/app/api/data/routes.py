...
from app.schemas import ImportKind, ImportResponse, OcrRequest, OcrRowsResponse, SourceType
...
@bp.get("/datasets/<dataset_id>/ocr-rows")
def list_ocr_rows(dataset_id: str):
    limit = _int_arg("limit", 10, 1, 100)
    offset = _int_arg("offset", 0, 0, 10**9)
    rows, total = get_data_service().list_ocr_rows(dataset_id, limit=limit, offset=offset)
    return json_response(OcrRowsResponse(rows=rows, total=total, limit=limit, offset=offset))


@bp.get("/datasets/<dataset_id>/assets/<import_id>")
def get_image_asset(dataset_id: str, import_id: str):
    data, filename, content_type = get_data_service().get_image_asset(dataset_id, import_id)
    return Response(
        data,
