...
class BrowserbaseResearchProvider:
    ...
    def create_research(self, dataset: Dataset, samples: list[str], research_type: str = "pos") -> ResearchArtifact:
        ...
        completion = self._complete_research_json(prompt, research_type, dataset.id)
        data = completion.data
    ...
    def _complete_research_json(self, prompt: str, research_type: str, dataset_id: str) -> JsonCompletion:
        system = (
            "You are a careful low-resource language research assistant. "
            "Return exactly one compact valid JSON object. Do not include markdown, code fences, or prose outside JSON."
        )
        try:
            ...
            if "invalid JSON" not in str(exc):
                raise
            print(
                "[research-debug] anthropic research invalid json; retrying with compact instructions "
                f"dataset_id={dataset_id} type={research_type} error={exc}",
                flush=True,
            )
            retry_prompt = json.dumps(
                {
                    ...
                    "instruction": (
                        ...
                        "No markdown, no code fences, no extra prose. Keep summary under 120 words, "
                        "guidelines to at most 5 strings, and examples to at most 3 items."
                    ),
            ...
            return self.llm.complete_json(
                task_name=f"{research_type}_research_retry",
                system=system,
                prompt=retry_prompt,
                max_tokens=self.research_max_tokens,
            )
        ...
        return json.dumps(
            {
                ...
                "instructions": [
                    ...
                    "Keep summary under 120 words, guidelines to at most 5 items, and examples to at most 3 items.",
                ],
                "output_limits": {
                    "summary_words_max": 120,
                    "guidelines_max": 5,
                    "examples_max": 3,
                    "strings": "short and reviewable",
                },
...
class PosAnnotationProvider:
    ...
    def _complete_pos_json(self, prompt: str) -> JsonCompletion:
        system = (
